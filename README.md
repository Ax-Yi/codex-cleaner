...
本项目可以使用 PyInstaller 打包成 Windows 单文件 exe。建议在项目根目录打开 PowerShell 执行下面的命令。
...
如果 PowerShell 提示脚本执行策略限制，可以临时允许当前窗口执行：
...
* `--onefile`：生成单文件 exe
* `--noconsole`：启动时不显示黑色控制台窗口，适合 Tkinter 图形界面
* `--name WindowsCleanerAssistant`：指定 exe 文件名

打包完成后，程序位于：

```text
dist\WindowsCleanerAssistant.exe
```

### 5. 不要提交或打包的文件

不要把扫描报告、IDE 配置、缓存、临时文件和打包产物提交到 GitHub，也不要通过 `--add-data` 把它们打进 exe。

项目已经在 `.gitignore` 中忽略这些内容：
...
本项目打包命令没有使用 `--add-data`，因此扫描报告、`.idea`、`__pycache__` 和临时文件不会作为项目资源被打进 exe。
